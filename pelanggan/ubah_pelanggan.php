...
<body>
    <?php 
    include "../koneksi.php";
    $qry_get_pelanggan=mysqli_query($conn,"select * from member where id = '".$_GET['id']."'");
    $dt_pelanggan=mysqli_fetch_array($qry_get_pelanggan);
    ?>
    <h3>Ubah Pelanggan</h3>
    <form action="proses_ubah_pelanggan.php" method="post">
        <input type="hidden" name="id" value= "<?=$dt_pelanggan['id']?>">
        Nama :
        ...
        <select name="jenis_kelamin" class="form-control">
            <option></option>
            <option value="Laki-Laki" <?php if($dt_pelanggan['jenis_kelamin']=='Laki-Laki'){echo "selected";} ?> >Laki-Laki</option>
            <option value="Perempuan" <?php if($dt_pelanggan['jenis_kelamin']=='Perempuan'){echo "selected";} ?> >Perempuan</option>
        </select>
        Telepon : 
